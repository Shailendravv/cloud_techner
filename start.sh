...
_install_backend_deps() {
    ...
    # One requirements file for every profile. It carries the runtime, uvicorn,
    # the test suite, the ingestion tools and the docs site, so whatever
    # APP_ENV is set to, the venv this produces can run it.
...
_load_env() {
    ...
    # APP_ENV selects config/<profile>.json, which decides the store backend
    # and the embedder, so it is resolved before anything else runs.
    if [ -z "${APP_ENV:-}" ]; then
...
#     Before the venv, because APP_ENV decides which config profile is loaded.
_load_env
