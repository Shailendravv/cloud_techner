...
def _pdf_to_markdown(raw: bytes) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            "`pymupdf4llm` is not installed. It is in requirements.txt - run "
            "`pip install -r requirements.txt`. It is only needed for "
            "PDF ingestion, never inside a request handler."
        ) from exc
