...
run with `backend/` as the working directory.
...
and activates `backend/.venv` if it is missing, installs `backend/requirements.txt`,
installs `frontend/node_modules` if it is missing or
